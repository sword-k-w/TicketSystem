...
private:
  ...
  struct node {
    ...
    node() = delete;
    node(const value_type &val, node *father = nullptr) : val_(val), father_(father) {}
    node(const node &other) : val_(other.val_), color_(other.color_) {}
    node *LeftMost() { return left_son_ != nullptr ? left_son_->LeftMost() : this; }
    node *RightMost() { return right_son_ != nullptr ? right_son_->RightMost() : this; }
    bool IsLeftSon() const { return father_->left_son_ == this; } // assure this != root_
    node *Brother() const { return IsLeftSon() ? father_->right_son_ : father_->left_son_; } // assure this != root_
...
public:
  ...
  */
  ...
  public:
    ...
    iterator(const iterator &other) : map_ptr_(other.map_ptr_), ptr_(other.ptr_) {}
    /**
    * TODO iter++
    */
    ...
    */
    bool operator==(const iterator &rhs) const { return map_ptr_ == rhs.map_ptr_ && ptr_ == rhs.ptr_; }
    bool operator==(const const_iterator &rhs) const { return map_ptr_ == rhs.map_ptr_ && ptr_ == rhs.ptr_; }
    /**
  ...
  class const_iterator {
    ...
    public:
      ...
      const_iterator(const map *map_ptr, const node *ptr) : map_ptr_(map_ptr), ptr_(ptr) {}
      const_iterator(const const_iterator &other) : map_ptr_(other.map_ptr_), ptr_(other.ptr_) {}
      const_iterator(const iterator &other) : map_ptr_(other.map_ptr_), ptr_(other.ptr_) {}
    ...
    while (cur != nullptr) {
      if (Compare()(pos->first, cur->val_.first)) {
        ...
        if (cur->left_son_ != nullptr && cur->right_son_ != nullptr) {
          // transform the case that remove a node with 2 sons to the case that remove a node with 0/1 son
          node *tmp = cur->left_son_->RightMost(); // tmp is the substitute of cur
        ...
        if (cur->left_son_ != nullptr && cur->right_son_ == nullptr) {
          ...
          while (true) {
            ...
            bool black_left = (brother->left_son_ == nullptr || brother->left_son_->color_ == kBlack);
            bool black_right = (brother->right_son_ == nullptr || brother->right_son_->color_ == kBlack);
